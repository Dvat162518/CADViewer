...
{
    return Quantity_Color(theColor.red()   / 255.0,
...
{
    NCollection_Vec3<double> anRgb;
    theColor.Values(anRgb.r(), anRgb.g(), anRgb.b(), Quantity_TOC_sRGB);
    return QColor((int)Round(anRgb.r() * 255.0),
                  (int)Round(anRgb.g() * 255.0),
                  (int)Round(anRgb.b() * 255.0));
}
...
{
    return Quantity_ColorRGBA(OcctQtTools::qtColorToOcct(theColor),
                              theColor.alpha() / 255.0f);
}
...
// ================================================================
void OcctQtTools::qtGlCapsFromSurfaceFormat(OpenGl_Caps& theCaps,
                                     ...
                                     const QTouchEvent* theEvent)
{
    ...
    {
        const Standard_Size aTouchId = (Standard_Size)aQTouch.id();
        const Graphic3d_Vec2d aNewPos2d =
            theView->Window()->ConvertPointToBacking(
                Graphic3d_Vec2d(aQTouch.position().x(), aQTouch.position().y()));
    ...
    for (const QTouchEvent::TouchPoint& aQTouch : theEvent->touchPoints())
    {
        ...
        const Graphic3d_Vec2d aNewPos2d =
            theView->Window()->ConvertPointToBacking(
                Graphic3d_Vec2d(aQTouch.pos().x(), aQTouch.pos().y()));
        const Graphic3d_Vec2i aNewPos2i = Graphic3d_Vec2i(aNewPos2d + Graphic3d_Vec2d(0.5));
